...
function updateEnemyInfoPanel(monster) {
  ...
  document.getElementById('panelEnemyStats').innerText =
    `HP: ${monster.baseStats?.hp || monster.health} | Mana: ${monster.baseStats?.mana || monster.mana} | Speed: ${monster.baseStats?.speed || '-'}`;
}
...
const availableEnemyAttacks = enemyMonster.attacks.filter((attack) => {
  if (attack.name === 'Fireball') {
    // Fireball hanya bisa dipilih jika:
    // - cukup mana
    // - sudah pernah basic attack dulu (usedBasicAttack === true)
    return enemyMonster.mana >= 25 && enemyMonster.usedBasicAttack
  }
  return true // serangan lain (misalnya Tackle) selalu boleh
})

// Pilih random dari yang valid
